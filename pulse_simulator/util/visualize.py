...
def tomography_data_for_aState(system, unitary, ini):
    """extract the hamiltonian tomography data
    Args:
        system (System) : class for the target quantum system
        unitary (list) : list of the np.array of the target unitary matrices
        ini (dict) : dict of the initial states like {0:"S0", 1:"S0", 2:"S0"}
    """
    inis = [ini]
    obss = []
    ...
    for idx in system.qubits.keys():
        d0 = []
        for obs in ["Ql","Qx","Qy","Qz"]:
            d0.append(data[str((ini, {idx:obs}))])
        shaped_data[idx] = np.array(d0)
    return shaped_data

def visualize_tomography_for_aState(time, data, trig_pos=None):
    """visualize the results of the tomography
    Args:
...
def hamiltonian_tomography_zx_data(system, unitary, control):
    """extract the hamiltonian tomography (zx) data
    Args:
        system (System) : class for the target quantum system
        unitary (list) : list of the np.array of the target unitary matrices
...
def hamiltonian_tomography_xz_data(system, unitary, control):
    """extract the hamiltonian tomography (xz) data
    Args:
        system (System) : class for the target quantum system
        unitary (list) : list of the np.array of the target unitary matrices
...
def hamiltonian_tomography_zz_data(system, unitary, control):
    """extract the hamiltonian tomography (zz) data
    Args:
        system (System) : class for the target quantum system
        unitary (list) : list of the np.array of the target unitary matrices
        control (int) : index of the control qubit be prepared + or - state
    """
    inis = [{control:"S0", "default":"Sp"}, {control:"S1", "default":"Sp"}]
    obss = []
    for idx, qubit in system.qubits.items():
        for obs in ["Ql","Qx","Qy","Qz"]:
            obss.append({idx:obs})
    ...
    for idx in system.qubits.keys():
        ...
        d1 = []
        for obs in ["Ql","Qx","Qy","Qz"]:
            d0.append(data[str(({control:"S0", "default":"Sp"}, {idx:obs}))])
            d1.append(data[str(({control:"S1", "default":"Sp"}, {idx:obs}))])
        shaped_data[idx] = np.array([d0, d1])
    return shaped_data

def visualize_tomography(time, data, trig_pos=None):
...
def _analyze_cr_pauli(time, data):
    ...
    vi  = 0.5*(v00 + v10)
    vz  = 0.5*(v00 - v10)
    pauli = np.hstack([vi,vz])
    return pauli

def analyze_hamiltonian_tomography_zx(time, data, idx, visualize=True):
    """analyze the results of the hamiltonian tomography (zx)
    Args:
        time (list or np.array) : simulation time
...
def analyze_hamiltonian_tomography_xz(time, data, idx, visualize=True):
    """analyze the results of the hamiltonian tomography (xz)
    Args:
        time (list or np.array) : simulation time
